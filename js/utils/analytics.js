...
console.log("Snapshot raw:", snapshotRaw);
...
if (!snapshotRaw) {
  ...
  document.getElementById("snapCall").textContent = ₹(snapshot.call.totalErosion);
  document.getElementById("snapPut").textContent = ₹(snapshot.put.totalErosion);

  document.getElementById("snapRatio").textContent =
    Math.abs(snapshot.call.totalErosion / snapshot.put.totalErosion).toFixed(2);
